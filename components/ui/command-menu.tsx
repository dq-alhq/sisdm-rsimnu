    ...
    return (
        ...
        >
            {isPending ? (
                ...
                <MagnifyingGlassIcon className='size-5 shrink-0 text-muted-fg' data-slot='command-menu-search-icon' />
            )}
            <Input
...
const CommandMenuSection = <T extends object>({ className, ref, ...props }: MenuSectionProps<T>) => {
    return (
        <MenuSection
            className={twMerge('col-span-full grid grid-cols-[auto_1fr] content-start gap-y-0.25', className)}
            ref={ref}
            {...props}
        >
            {'label' in props && (
...
const CommandMenuItem = ({ className, ...props }: React.ComponentProps<typeof MenuItem>) => {
    ...
    return <MenuItem {...props} className={cx('items-center gap-y-0.5', className)} textValue={textValue} />
}
