...
import { IconCalendarClock, IconFloppyDisk } from '@intentui/icons'
...
export const FormAbsensi = () => {
    ...
    const [checkInAt, setCheckinAt] = useState(normalizeTime('00:00'))
    const [checkOutAt, setCheckoutAt] = useState(normalizeTime('00:00'))
    const [late, setLate] = useState('00:00')
    const [earlyDeparture, setEarlyDeparture] = useState('00:00')
    ...
    const onTriggerPress = async () => {
        setOpen(true)
        setCheckinAt(normalizeTime('00:00'))
        setCheckoutAt(normalizeTime('00:00'))
        setLate('00:00')
        setEarlyDeparture('00:00')
        setTotalWorkHours('00:00')
        setShiftCode('00:00')
        ...
        if (employees.length === 0) {
            const res = await listEmployeesDepartment()
    ...
    return (
        <Modal isOpen={open} onOpenChange={setOpen}>
            <Button onPress={onTriggerPress}>
                <IconCalendarClock />
                Tambah Manual
            </Button>
            <ModalContent>
                <ModalHeader>
                    <ModalTitle>Tambah Absensi Manual</ModalTitle>
                    <ModalDescription>Tambah absensi untuk pegawai</ModalDescription>
                </ModalHeader>
